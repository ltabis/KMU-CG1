...
void CG::Test::TestShaderEditor::onRender()
{
	...
	m_PhongShader->setUniform("u_mvp", m_Controller->projectionView() * m_Sphere->transform.model());
	m_PhongShader->setUniform("u_view", m_Controller->view());
	m_PhongShader->setUniform("u_modelView", m_Controller->view() * m_Sphere->transform.model());
	m_PhongShader->setUniform("u_normalMat", normalMat);
	_renderer->draw(*m_Sphere, *m_PhongShader);

	normalMat = glm::mat3(glm::transpose(glm::inverse(m_Controller->view() * m_TeaPot->transform.model())));

	m_PhongShader->setUniform("u_mvp", m_Controller->projectionView() * m_TeaPot->transform.model());
	m_PhongShader->setUniform("u_view", m_Controller->view());
	m_PhongShader->setUniform("u_modelView", m_Controller->view() * m_TeaPot->transform.model());
	m_PhongShader->setUniform("u_normalMat", normalMat);
	_renderer->draw(*m_TeaPot, *m_PhongShader);
